...
function severityConfig(score) {
    if (score <= 3)
        return { accent: '#EF4444', bg: 'rgba(239,68,68,0.06)', label: 'Critical' };
    if (score <= 5)
        return { accent: '#F59E0B', bg: 'rgba(245,158,11,0.06)', label: 'Needs Attention' };
    if (score <= 8)
        return { accent: '#6366F1', bg: 'rgba(99,102,241,0.06)', label: 'Info' };
    return { accent: '#10B981', bg: 'rgba(16,185,129,0.06)', label: 'Strong' };
}
function stageLabel(stage) {
    const s = (stage || '').toLowerCase();
    if (s.includes('strong'))
        return { text: 'Strong PMF', color: '#059669', bg: '#ecfdf5' };
    if (s.includes('early'))
        return { text: 'Early PMF', color: '#0891b2', bg: '#ecfeff' };
    if (s.includes('approach'))
        return { text: 'Approaching PMF', color: '#d97706', bg: '#fffbeb' };
...
function renderScorecardDetail(data) {
    const content = data.content || {};
...
function renderMarketAnalysis(content) {
...
// ── Competitors ──────────────────────────────────────────────────────────────
function renderCompetitors(content) {
...
function renderStrategy(content) {
...
function renderBottomLine(data) {
    ...
    const nextSteps = (typeof bl === 'object' ? bl.nextSteps : null) || [];
    const stage = stageLabel(data.pmfStage);
    const color = scoreColor(data.pmfScore, 100);
...
function renderSourcesInline(sources) {
    ...
    if (Array.isArray(sources)) {
        items = sources.map((s) => typeof s === 'string' ? s : s.url || s.title || JSON.stringify(s));
    }
    else if (typeof sources === 'object' && sources.items) {
        items = sources.items.map((s) => typeof s === 'string' ? s : s.url || s.title || JSON.stringify(s));
    }
    else {
