...
exports.classifyAssessment = classifyAssessment;
const ai_service_1 = require("./ai.service");
const classification_schema_1 = require("../schemas/classification.schema");
...
function formatFounderAnswers(answers) {
    return [
...
function buildClassificationMessages(answers) {
    return [
...
async function classifyAssessment(assessmentId) {
    ...
    if (assessment.status !== 'in_progress') {
        throw new errors_1.ValidationError(`Assessment must be in_progress to classify. Current status: ${assessment.status}`);
    }
    ...
    const answers = {
        ...
        q3_distribution: assessment.responses.find((r) => r.questionOrder === 3)?.answerValue ||
    ...
    if (!q1Trimmed || q1Trimmed.length < 10) {
        throw new errors_1.ValidationError('Product description (Q1) is required and must be at least 10 characters.');
    }
    ...
    const result = await (0, ai_service_1.callOpenAI)({
        ...
        promptName: 'classify',
        messages,
        responseFormat: classification_schema_1.classificationResponseFormat,
        temperature: 0.2,
    });
    ...
    try {
        ...
        throw new errors_1.AIError('Classification returned invalid JSON');
    }
    // 8. Check for error response (garbage input detected by LLM)
    if (typeof parsed === 'object' && parsed !== null && 'error' in parsed) {
        throw new errors_1.ValidationError(`Classification rejected input: ${parsed.error}`);
    }
    // 9. Validate with Zod schema
    const validated = classification_schema_1.classificationOutputSchema.safeParse(parsed);
    if (!validated.success) {
        throw new errors_1.AIError(`Classification output failed schema validation: ${validated.error.message}`);
    }
