...
exports.generateReportWithCorrections = generateReportWithCorrections;
...
// ============================================================================
function buildReportUserMessage(founderAnswers, research, scores, classificationData, researchQuality) {
    ...
    if (processedResearch.competitors) {
        processedResearch.competitors.forEach((c) => {
            if (c.sourceUrl)
                sourceUrls.add(c.sourceUrl);
            // Only mark strictly numeric fields as "Data not available"
            if (c.g2Rating === null)
                c.g2Rating = "Not found on G2";
            if (c.reviewCount === null)
                c.reviewCount = "Not found on G2";
            if (c.funding === null)
                c.funding = "Not publicly disclosed";
            if (c.pricingModel === null)
                c.pricingModel = "Not found";
            if (c.freeTier === null)
                c.freeTier = "Unknown";
        });
    }
    if (processedResearch.market) {
        if (processedResearch.market.tam === null)
            processedResearch.market.tam = "No TAM data found in research";
        if (processedResearch.market.sam === null)
            processedResearch.market.sam = "No SAM data found in research";
        if (processedResearch.market.growthRate === null)
            processedResearch.market.growthRate = "No growth rate found in research";
    }
    if (processedResearch.complaints) {
        processedResearch.complaints.forEach((c) => {
            if (c.percentage === null)
                c.percentage = "Not quantified";
            if (c.sourceUrl)
                sourceUrls.add(c.sourceUrl);
        });
    }
    if (processedResearch.patterns?.topCompanies) {
        processedResearch.patterns.topCompanies.forEach((c) => {
            if (c.salesModel === null)
                c.salesModel = "Not determined";
            if (c.positioning === null)
                c.positioning = "Not found";
            if (c.sourceUrl)
                sourceUrls.add(c.sourceUrl);
        });
    }
...
// ============================================================================
function validateContent(report, scores) {
    const errors = [];
    ...
    for (const inputDim of scores.dimensions) {
        const reportDim = report.scorecard.dimensions.find((s) => s.name.toLowerCase() === inputDim.dimension.toLowerCase());
        if (!reportDim) {
            errors.push(`Scorecard missing dimension: ${inputDim.dimension}`);
        }
        else if (reportDim.score !== inputDim.score) {
            errors.push(`Scorecard score mismatch for ${inputDim.dimension}: got ${reportDim.score}, expected ${inputDim.score}`);
        }
    }
    // pmf_score must match
    if (report.header.pmf_score !== scores.pmfScore) {
        errors.push(`Header pmf_score ${report.header.pmf_score} does not match input ${scores.pmfScore}`);
    }
    // pmf_stage must match
    if (report.header.pmf_stage !== scores.pmfStage) {
        errors.push(`Header pmf_stage "${report.header.pmf_stage}" does not match input "${scores.pmfStage}"`);
    }
    ...
    if (errors.length > 0) {
        throw new errors_1.AIError(`Report content validation failed: ${errors.join('; ')}`);
    }
}
// ============================================================================
...
async function generateReport(params) {
    const researchQuality = typeof params.research.researchQuality === 'object'
        ? params.research.researchQuality
        : undefined;
    ...
    const result = await (0, ai_service_1.callOpenAI)({
        ...
        responseFormat: report_schema_1.reportResponseFormat,
        temperature: 0.1,
        maxTokens: 16000,
    });
    // Parse JSON
    ...
    try {
        ...
        throw new errors_1.AIError('Report generation returned invalid JSON');
    }
    // Zod structural validation
    const parseResult = report_schema_1.reportOutputSchema.safeParse(parsed);
    if (!parseResult.success) {
        const issues = parseResult.error.issues.map((i) => `${i.path.join('.')}: ${i.message}`);
        throw new errors_1.AIError(`Report schema validation failed: ${issues.join('; ')}`);
    }
...
// ============================================================================
async function generateReportWithCorrections(params, previousFlags) {
    const researchQuality = typeof params.research.researchQuality === 'object'
        ? params.research.researchQuality
        : undefined;
    const userMessage = buildReportUserMessage(params.founderAnswers, params.research, params.scores, params.classificationData, researchQuality);
    ...
    try {
        ...
        throw new errors_1.AIError('Report correction returned invalid JSON');
    }
    // Zod structural validation
    const parseResult = report_schema_1.reportOutputSchema.safeParse(parsed);
    if (!parseResult.success) {
