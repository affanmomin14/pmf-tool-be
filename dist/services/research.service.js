...
const research_schema_1 = require("../schemas/research.schema");
...
// ============================================================================
async function searchCompetitors(ctx) {
    try {
        const competitorHints = ctx.likelyCompetitors.length > 0
            ...
            : '';
        const result = await (0, ai_service_1.callOpenAIWebSearch)({
            assessmentId: ctx.assessmentId,
            promptName: 'research_competitors',
            model: env_1.env.OPENAI_RESEARCH_MODEL,
...
CRITICAL: Only include competitors that actually appear in search results. Never invent companies. Use null for any field where data was not explicitly found.`,
            ...
            textFormat: (0, zod_1.zodTextFormat)(research_schema_1.competitorExtractionSchema, 'competitor_extraction'),
            temperature: 0.1,
        });
        const parsed = JSON.parse(result.outputText);
        logger_1.logger.info(`[research] Competitors found: ${parsed.competitors?.length ?? 0} for ${ctx.category}/${ctx.subCategory}`);
        return parsed.competitors;
    }
    catch (err) {
        logger_1.logger.error(`[research] searchCompetitors failed for ${ctx.category}/${ctx.subCategory}: ${err instanceof Error ? err.message : String(err)}`);
        return [];
    }
}
async function searchMarketData(ctx) {
    try {
        const result = await (0, ai_service_1.callOpenAIWebSearch)({
            assessmentId: ctx.assessmentId,
...
Use null for any field not found. Never invent or estimate data.`,
            searchContextSize: 'high',
            textFormat: (0, zod_1.zodTextFormat)(research_schema_1.marketExtractionSchema, 'market_extraction'),
            temperature: 0.1,
        });
        const parsed = JSON.parse(result.outputText);
        logger_1.logger.info(`[research] Market data found: TAM=${parsed.market?.tam ?? 'null'}, growth=${parsed.market?.growthRate ?? 'null'}`);
        return parsed.market;
    }
    catch (err) {
        logger_1.logger.error(`[research] searchMarketData failed for ${ctx.category}/${ctx.subCategory}: ${err instanceof Error ? err.message : String(err)}`);
        return { tam: null, sam: null, growthRate: null, regions: null };
    }
}
async function searchComplaints(ctx) {
    try {
...
CRITICAL: Only report complaints that are explicitly mentioned in search results. Do NOT estimate or fabricate complaint percentages. Set percentage to null for every complaint.`,
            ...
            textFormat: (0, zod_1.zodTextFormat)(research_schema_1.complaintExtractionSchema, 'complaint_extraction'),
            temperature: 0.1,
        });
        const parsed = JSON.parse(result.outputText);
        logger_1.logger.info(`[research] Complaints found: ${parsed.complaints?.length ?? 0}`);
        return parsed.complaints;
    }
    catch (err) {
        logger_1.logger.error(`[research] searchComplaints failed for ${ctx.category}/${ctx.subCategory}: ${err instanceof Error ? err.message : String(err)}`);
        return [];
    }
}
async function searchPatterns(ctx) {
    try {
        const competitorHints = ctx.likelyCompetitors.length > 0
...
CRITICAL: Only report companies and patterns from search results. Use null for any field not found.`,
            searchContextSize: 'medium',
            textFormat: (0, zod_1.zodTextFormat)(research_schema_1.patternExtractionSchema, 'pattern_extraction'),
            temperature: 0.1,
        });
        const parsed = JSON.parse(result.outputText);
        logger_1.logger.info(`[research] Patterns found: ${parsed.patterns?.topCompanies?.length ?? 0} companies, ${parsed.patterns?.gaps?.length ?? 0} gaps`);
        return parsed.patterns;
    }
    catch (err) {
        logger_1.logger.error(`[research] searchPatterns failed for ${ctx.category}/${ctx.subCategory}: ${err instanceof Error ? err.message : String(err)}`);
...
// ============================================================================
async function seedCompetitorsFromClassification(ctx) {
    if (ctx.likelyCompetitors.length === 0)
        return [];
    try {
...
CRITICAL: Search for each company individually. Only include companies you can verify exist. Return at least basic info (name + tagline) for each verified company.`,
        ...
        const parsed = JSON.parse(result.outputText);
        logger_1.logger.info(`[research] Seeded competitors from classification: ${parsed.competitors?.length ?? 0}`);
        return parsed.competitors;
    }
    catch (err) {
        logger_1.logger.error(`[research] seedCompetitorsFromClassification failed: ${err instanceof Error ? err.message : String(err)}`);
        return [];
    }
}
// ============================================================================
...
// ============================================================================
function computeResearchQuality(competitors, market, complaints) {
 ...
 */
async function runResearch(assessmentId, forceRefresh = false) {
    ...
    if (assessment.status !== 'completed') {
        throw new errors_1.ValidationError('Assessment must be completed (classified) before research');
    }
    ...
    if (!classification?.category || !classification?.sub_category) {
        throw new errors_1.ValidationError('Assessment must be classified before research');
    }
    const { category, sub_category: subCategory, search_queries: searchQueries } = classification;
    const likelyCompetitors = classification.likely_competitors || [];
    // 4. Extract founder answers for richer context
    const getAnswer = (order) => {
        const r = assessment.responses.find((r) => r.questionOrder === order);
        return r?.answerText || r?.answerValue || '';
    };
    const q3Response = assessment.responses.find((r) => r.questionOrder === 3);
    const ctx = {
        category,
        subCategory,
    ...
    if (!forceRefresh) {
        const cached = await prisma_1.prisma.researchCache.findUnique({
            where: { category_subCategory: cacheKey },
        });
        if (cached && cached.expiresAt > new Date()) {
            const cachedData = cached.data;
            const quality = computeResearchQuality(cachedData.competitors || [], cachedData.market || { tam: null, sam: null, growthRate: null, regions: null }, cachedData.complaints || []);
            ...
            logger_1.logger.info(`[research] Cached data quality is 'minimal' — bypassing cache for ${assessmentId}`);
        }
    }
    // 7. Run all 4 search dimensions in parallel
    const start = Date.now();
    logger_1.logger.info(`[research] Starting parallel searches for ${category}/${subCategory} (${likelyCompetitors.length} known competitors)`);
    ...
    let callCount = 4;
    logger_1.logger.info(`[research] Initial quality: ${researchQuality.overall} (competitors=${researchQuality.competitorCount}, market=${researchQuality.hasMarketData}, complaints=${researchQuality.complaintCount})`);
    ...
    if (competitors.length === 0 && likelyCompetitors.length > 0) {
        logger_1.logger.info(`[research] Retry L1: Seeding competitors from classification (${likelyCompetitors.length} known)`);
    ...
    if (researchQuality.overall === 'minimal' || researchQuality.overall === 'thin') {
        logger_1.logger.info(`[research] Retry L2: Broadened queries (quality=${researchQuality.overall})`);
        const retryPromises = [];
        if (competitors.length < 3) {
            retryPromises.push((async () => {
                ...
                if (retried.length > competitors.length)
                    competitors = retried;
                callCount++;
            })());
        }
        ...
        if (complaints.length < 2) {
            retryPromises.push((async () => {
                ...
                if (retried.length > complaints.length)
                    complaints = retried;
                callCount++;
            })());
        }
        if (!patterns.topCompanies || patterns.topCompanies.length === 0) {
            retryPromises.push((async () => {
                ...
                if (retried.topCompanies && retried.topCompanies.length > 0)
                    patterns = retried;
                callCount++;
            })());
        }
        ...
        researchQuality = computeResearchQuality(competitors, market, complaints);
        logger_1.logger.info(`[research] After L2 retries: quality=${researchQuality.overall} (competitors=${researchQuality.competitorCount}, market=${researchQuality.hasMarketData})`);
    }
    // Level 3: Last resort — use broader parent category for market data
    if (!market.tam) {
        logger_1.logger.info(`[research] Retry L3: Searching broader category "${category}" for market data`);
        const broadCtx = {
            ...ctx,
            searchQueries: [
    ...
    if (competitors.length === 0) {
        logger_1.logger.info(`[research] Retry L4: Very broad competitor search`);
        const broadCtx = {
            ...ctx,
            searchQueries: [
    ...
    logger_1.logger.info(`[research] Final quality: ${researchQuality.overall} (competitors=${researchQuality.competitorCount}, market=${researchQuality.hasMarketData}, complaints=${researchQuality.complaintCount}) in ${totalDurationMs}ms with ${callCount} calls`);
    ...
    const validated = research_schema_1.researchOutputSchema.safeParse(researchOutput);
    if (!validated.success) {
        throw new errors_1.AIError(`Research output failed validation: ${validated.error.message}`);
    }
    // 12. Store in cache with 7-day TTL
    const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
    await prisma_1.prisma.researchCache.upsert({
        where: { category_subCategory: cacheKey },
        update: { data: validated.data, expiresAt },
        create: { ...cacheKey, data: validated.data, expiresAt },
    });
    // 13. Store snapshot on assessment
    await prisma_1.prisma.assessment.update({
        where: { id: assessmentId },
        data: { researchData: validated.data },
    });
